...
Once 1 killer + 4 survivors have joined: the embed locks, a thread ( match-0000 ) opens
with the recap, and a voice channel ( Match 0000 ) is created for just those 5.
...
/cancel killer’s confirmation.
(inside a match thread) — vote to cancel. Needs 3 of 4 survivors plus the
...
Losing-side values weren’t specified in the original spec, so they mirror the winner’s tier —
change SURVIVOR_LOSS_* / KILLER_LOSS_* in config.py if you want different numbers.
Ranks: Bronze 0-199, Silver 200-299, Gold 300-399, Platinum 400-499, Diamond 500-
599, Champion 600+.
Files
bot.py — entry point, intents, cog loading
config.py — every name/number you’re likely to want to tweak
database.py — Postgres (asyncpg) data layer
embeds.py / views.py — embed rendering and buttons
elo_utils.py / match_logic.py — elo math and match-resolution logic
queue_cog.py , scoring_cog.py, elo_cog.py, leaderboard_cog.py,
moderation_cog.py 
